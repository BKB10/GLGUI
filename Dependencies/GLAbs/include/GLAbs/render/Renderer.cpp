...
void Loader::deleteVao(VaoData* vaoData) {
    for (unsigned int i = 0; i < vaoDatas.size(); i++) {
        if (vaoDatas[i]->rawModel->vaoId == vaoData->rawModel->vaoId) {
            vaoDatas.erase(vaoDatas.begin() + i);
...
void VaoData::cleanUp() {
    if (rawModel != nullptr) {
        glDeleteVertexArrays(1, &rawModel->vaoId);
    }
    ...
    if (modelTexture != nullptr) {
        glDeleteBuffers(1, &modelTexture->textureId);
    }
}
